...
class MMDLatentDiffusion(pl.LightningModule):
    def __init__(self,
                 model,
                 autoencoder,
        ...
        self.mmd_c = mmd_c
        if self.mmd_c>0:
            self.mmd_loss_function = MMD_loss()
            self.use_mmd = True
        else:
            self.mmd_loss_function = None
            self.use_mmd = False
    ...
    def p_losses(self, x_start, t, noise=None, context=None):
        ...
        return self.get_loss(model_out, target), yhat
    ...
    def compute_mmd(self, pred, target):
        tx, ty, tb = torch.randint(pred.shape[-1]-16, (1,)), torch.randint(pred.shape[-2]-16, (1,)), torch.randint(pred.shape[0], (1,))
        mmd_losses = []
        for t in range(pred.shape[2]):
            mmd_losses.append(self.mmd_loss_function(pred[tb, :, t, ty:ty+16, tx:tx+16].contiguous().view(-1,1),
                                                     target[tb, :, t, ty:ty+16, tx:tx+16].contiguous().view(-1,1)).contiguous().view((1)))
        return torch.mean(torch.cat(mmd_losses))
    ...
    def shared_step(self, batch):
        ...
        loss, yhat = self(self.autoencoder.encode(y)[0], context=context)
        if self.use_mmd:
            yhat = self.autoencoder.decode(yhat)
            mmd_loss = self.compute_mmd(yhat, y)
        else:
            mmd_loss = None
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, mmd_loss = self.shared_step(batch)
        if self.use_mmd:
            loss = (1-self.mmd_c)*loss + self.mmd_c*mmd_loss
            self.log("mmd_loss", mmd_loss, **log_params)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_mmd:
            loss = (1-self.mmd_c)*loss + self.mmd_c*mmd_loss
            self.log("val_mmd_loss", mmd_loss, **log_params)
        with self.ema_scope():
            loss_ema, mmd_loss_ema = self.shared_step(batch)
            if self.use_mmd:
                loss_ema = (1-self.mmd_c)*loss + self.mmd_c*mmd_loss_ema
                self.log("val_mmd_loss_ema", mmd_loss_ema, **log_params)
